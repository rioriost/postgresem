# Release-candidate deprecation policy

The M11 contract inventory is the candidate compatibility boundary for 1.0.
...
| additive CLI command or MCP tool | requires tests, documentation, and manifest refresh |
...
| removal of a deprecated surface | prohibited before its documented removal boundary |

## Current classifications

- **Frozen candidate:** LSQ v1, LSM v1, Semantic Snapshot v1/v2 loading,
...
- **Deprecated but supported:** `postgresem report beta`. Use
  `postgresem report operations`. No removal version is assigned before 1.0.
...
5. refresh the RC manifest intentionally with
